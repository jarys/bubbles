...
def line(a, b, color=(0,0,0,0), stroke=0, style=0):
	"""Draw line"""
	glColor4f(*color)
	...
	glBegin(GL_LINES)
	glVertex2i(int(a[0]), int(a[1]))
	glVertex2i(int(b[0]), int(b[1]))
	glEnd()
...
def circle(pos=(10, 10), r=20, color=(0,0,0,1), stroke=0, style=GLU_FILL):
	""" Draw Circle
		x, y, z, width in pixel, color and line width in px
	...
	gluDisk(q, inner, r, 60, 1) # gluDisk(quad, inner, outer, slices, loops)
		
	glPopMatrix()

def rect(pos, size, color=(0, 0, 0, 1)):
	glColor4f(*color)
	x, y = pos
	w, h = size
...
class Circle(Base):
	...
	def render(self):
		...
		if self.stroke :
			inner = self.r - self.stroke # outline width
			if inner < 0: inner=0
		else :
			 inner = 0 # filled
		...
		gluDisk(self.q, inner, self.r, self.circleresolution, 1) # gluDisk(quad, inner, outer, slices, loops)
...
class Arc(Base):
	""" Arc class
		Arc(x=10, y=10, z=0, r=1, start=0, sweep=1, color=(0,0,0,1), stroke=0, rotation=0.0, style=GLU_FILL)
		style choices are : GLU_LINE, GLU_FILL, GLU_SILHOUETTE, GLU_POINT
	"""
	def __init__(self, x=10, y=10, z=0, r=1, start=0, sweep=1, color=(0,0,0,1), stroke=0,
	...
	def render(self):
		...
		gluPartialDisk(self.q, inner, self.r, self.circleresolution, 1, self.start, self.sweep)
...
if __name__=="__main__":
	...
	win = pyglet.window.Window(config=config, resizable=True)
	...
	a = Arc(150,150,r=100,color=(1.,0.,0.,1.),sweep=90,style=GLU_FILL)
